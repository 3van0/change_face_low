...
class HeadPos:
    ...
    def getImagePoints(self, img):
        ...
        if 0 == len(dets):
            ...
            return -1, None
        largestIndex = self.__largestFace(dets)
        faceRectangle = dets[largestIndex]
        landmarkShape = self.predictor(img, faceRectangle)
        self.currentLandmark = landmarkShape

        return self.getImagePointsFromLandmarkShape(landmarkShape, self.POINTS_NUM_LANDMARK)
    ...
    def update(self):
        ...
        if (self.cap.isOpened()):
            ...
            if size[0] > 10000:
                ...
                w = size[1] / 3
                self.im = cv2.resize(
                    self.im, (int(w), int(h)), interpolation=cv2.INTER_CUBIC)
                size = self.im.shape
            ...
            ret, rotationVector, translationVector, cameraMatrix, dist_coeffs = self.getPoseEstimation(
                size, imagePoints)
            ...
            self.isUpdated = True
            self.currentData = (
                rotationVector, translationVector, [pitch, yaw, roll])
            return True, rotationVector, translationVector, [pitch, yaw, roll]

    # get current image (may or may not contain face)
    ...
    # get currentLandmark
    def getCurrentLandmark(self):
        if (self.isUpdated):
            return True, self.currentLandmark
        else:
            return False, None

    # get current feather points (6 points)
    def getCurrentPoins(self):
        if (self.isUpdated):
            return True, self.currentPoints
        else:
            return False, None

    # get a rectangular area containing the face.
    # use "expand=True" to expand the area.
    def getFaceBox(self, expand=False, w_rate=1, h_rate=1):
        if (self.isUpdated):
            ...
            if(expand):
                ...
                exp_point1, exp_point2 = self.expandWidth(
                    box[0], box[3], w_rate, deg)
                exp_point3, exp_point4 = self.expandHeight(
                    exp_point1, exp_point2, h_rate*height, deg)
            ...
            return True, steady_box, deg
        else:
            return False, None, None

    @staticmethod
    # fetch biggest face
    def __largestFace(dets):
        if len(dets) == 1:
            return 0

        faceAreas = [(det.right()-det.left())*(det.bottom()-det.top())
                     for det in dets]
    ...
    def getImagePointsFromLandmarkShape(landmarkShape, pointsNum):
        if landmarkShape.num_parts != pointsNum:
        ...
        imagePoints = np.array([
            ...
            (landmarkShape.part(8).x,
             landmarkShape.part(8).y),     # Chin
            (landmarkShape.part(36).x, landmarkShape.part(
                36).y),     # Left eye left corner
            (landmarkShape.part(45).x, landmarkShape.part(
                45).y),     # Right eye right corne
            (landmarkShape.part(48).x, landmarkShape.part(
                48).y),     # Left Mouth corner
            (landmarkShape.part(54).x, landmarkShape.part(
    ...
    def getPoseEstimation(imgSize, imagePoints):
        ...
        (success, rotationVector, translationVector) = cv2.solvePnP(modelPoints,
                                                                    imagePoints, cameraMatrix, dist_coeffs, flags=cv2.SOLVEPNP_ITERATIVE)
    ...
    @staticmethod
    # expand box width
    def expandWidth(point1, point2, rate, deg):
        ...
        if(point1[1] == point2[1]):
            ...
            point3 = [int(point1[0] + diff*math.sin(deg*math.pi/180)),
                      int(point1[1] + diff*math.cos(deg*math.pi/180))]
            point4 = [int(point2[0] - diff*math.sin(deg*math.pi/180)),
                      int(point2[1] + diff*math.cos(deg*math.pi/180))]
        else:
            point3 = [int(point1[0] - diff*math.cos(deg*math.pi/180)),
                      int(point1[1] - diff*math.sin(deg*math.pi/180))]
            point4 = [int(point2[0] + diff*math.cos(deg*math.pi/180)),
    ...
    def expandHeight(point1, point2, height, deg):
        if(point1[1] == point2[1]):
            ...
            point3 = [int(point1[0] + height*math.cos(deg*math.pi/180)),
                      int(point1[1] + height*math.sin(deg*math.pi/180))]
            point4 = [int(point2[0] + height*math.cos(deg*math.pi/180)),
                      int(point2[1] + height*math.sin(deg*math.pi/180))]
        else:
            point3 = [int(point1[0] + height*math.sin(deg*math.pi/180)),
                      int(point1[1] - height*math.cos(deg*math.pi/180))]
            point4 = [int(point2[0] + height*math.sin(deg*math.pi/180)),
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        (nose_end_point2D, jacobian) = cv2.projectPoints(np.array(
            [(0.0, 0.0, 1000.0)]), rotationVector, translationVector, cameraMatrix, dist_coeffs)
        ...
        cv2.putText(im, eulerAngle_str, (0, 120),
